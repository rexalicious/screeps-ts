...
module.exports = function(room) {
    function buildWorkingHarvester(opts) {
        ...
        var build = { WORK: 1, CARRY: 1, MOVE: 2 }
        var maxMultiplier = 3;
        
        var cost = getBuildCost(build);
        var multiplier = Math.min(maxMultiplier, Math.floor(opts.energy / cost));
    ...
    function buildHarvester(opts) {
        ...
        var build = { CARRY: 2, MOVE: (opts.roads ? 1 : 2) };
        var maxMultiplier = Math.ceil(Math.min(10, room.energyCapacityAvailable / (8 * CARRY_CAPACITY)));
    ...
    function buildToArray(build) {
        var order = [TOUGH,CLAIM,WORK,CARRY,MOVE,ATTACK,RANGED_ATTACK,HEAL];
