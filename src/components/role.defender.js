...
module.exports = {
    run: function(creep, myRooms) {
        ...
        if (!target) {
            var hostiles = _.flatten(_.map(_.values(myRooms), (r) => { return r.find(FIND_HOSTILE_CREEPS)}));
            
            var closest = creep.pos.findClosestByRange(hostiles);
            if (!closest) {
                var search = PathFinder.search(creep.pos, _.map(hostiles, (s) => { return { pos: s.pos, range: 1 } }));
                if (!search.incomplete && search.path.length > 0) {
                    let endPos = search.path[search.path.length - 1];
                    if (Game.rooms[endPos.roomName]) {
                        var creeps = endPos.findInRange(FIND_HOSTILE_CREEPS, 1);
        ...
        var dropped = creep.pos.findInRange(FIND_DROPPED_RESOURCES, 1, { filter: (r) => { return r.resourceType != RESOURCE_ENERGY; }});
